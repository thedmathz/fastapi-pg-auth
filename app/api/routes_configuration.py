...
async def post(configuration: ConfigurationPost, db: AsyncSession = Depends(get_db)):
...
async def put(id: int, configuration_update: ConfigurationPut, db: AsyncSession = Depends(get_db)):
