...
async def post(user: UserPost, db: AsyncSession = Depends(get_db), userID: str = Depends(svc_authentication.currentUserID)):
...
async def put(id: int, user_update: UserPut, db: AsyncSession = Depends(get_db), userID: str = Depends(svc_authentication.currentUserID)):
